...
// Define type for the JWT payload user from auth.ts
...
async function updateServiceRequest(req: NextRequest, user?: JwtPayload) {
  ...
  try {
    ...
    if (validatedData.location?.coordinates) {
      // Create a properly typed coordinates object
      const geoCoordinates = {
        longitude: validatedData.location.coordinates.longitude,
        latitude: validatedData.location.coordinates.latitude
      };
      validatedData.location.coordinates = geoCoordinates;
    }
